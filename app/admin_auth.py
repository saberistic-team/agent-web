...
LIMITER_DOMAIN_LEGACY_COMPOSITE = "legacy"
...
def _active_limiter_secrets(settings: Settings) -> tuple[str, ...]:
    ...
    return tuple(secrets_list)


def _limiter_keys_for_material(
    domain: str,
    material: str,
    settings: Settings,
) -> tuple[str, ...]:
    ...
    keys = [
        _digest_limiter_key(domain, normalized, secret)
...
def build_rate_limit_key(username: str, client_source: str, settings: Settings) -> str:
    ...
    return _digest_limiter_key(
        LIMITER_DOMAIN_LEGACY_COMPOSITE,
        material,
        settings.admin_login_limiter_secret,
    )
...
def login_limiter_keys(
    *,
    ...
    keys: list[str] = []
    keys.extend(
        _limiter_keys_for_material(LIMITER_DOMAIN_SOURCE, client_source, settings)
    )
...
def finalize_successful_login(request: Request, settings: Settings, *, username: str = "") -> None:
    ...
    _ = username
    source = client_ip(request, settings)
    source_keys = _limiter_keys_for_material(LIMITER_DOMAIN_SOURCE, source, settings)
    account_keys = (
        _limiter_keys_for_material(
            LIMITER_DOMAIN_ACCOUNT,
