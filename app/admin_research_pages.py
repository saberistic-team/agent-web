...
from app.admin_layout import render_admin_archive_restore_button, render_admin_shell
...
def render_admin_company_research_page(
    *,
    ...
    archive_button = render_admin_archive_restore_button(label=archive_label, archived=is_archived)
    error_html = ""
    if error_message:
        error_html = (
    ...
    main = f"""        <section class="admin-research" aria-labelledby="company-research-title">
          ...
          <form class="admin-record-action" method="post" action="/admin/companies/{company_id}/{archive_action}">
            <input type="hidden" name="csrf_token" value="{html.escape(csrf_token, quote=True)}" />
            {archive_button}
          </form>
...
def render_admin_contact_research_page(
    *,
    ...
    main = f"""        <section class="admin-research" aria-labelledby="contact-research-title">
          ...
          <dl class="research-provenance">{facts_html}</dl>
          <form class="admin-record-action" method="post" action="/admin/contacts/{contact_id}/{archive_action}">
            <input type="hidden" name="csrf_token" value="{html.escape(csrf_token, quote=True)}" />
            {archive_button}
          </form>
          <h2 class="admin-section-heading">Attach record</h2>
