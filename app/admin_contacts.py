...
def _role_checkboxes(selected: list[str] | None) -> str:
    ...
    return "\n".join(
        ...
        f'{" checked" if key in selected else ""} /> {_esc(label)}</label>'
        for key, label in BUYING_ROLES.items()
    )


def _company_options(companies: list[dict[str, Any]], selected: str | None) -> str:
    rows = [f'<option value="">{html.escape("Unassigned")}</option>']
    rows.extend(
        f'<option value="{_esc(company["id"])}"{" selected" if str(company.get("id")) == str(selected) else ""}>'
...
def _contact_form(
    *,
    ...
    email_permitted = contact.get("email_permitted")
    return f"""<form class="admin-form" method="post" action="{_esc(action)}">
      ...
      <div class="field"><label><input type="checkbox" name="email_permitted" value="1"{" checked" if email_permitted else ""} /> Email outreach permitted</label></div>
      <div class="field"><label for="email_source">Email source</label><select id="email_source" name="email_source">{_options(EMAIL_SOURCES, contact.get("email_source"), empty="Unspecified")}</select></div>
      <div class="field"><label for="last_interaction_at">Last interaction</label><input id="last_interaction_at" name="last_interaction_at" type="datetime-local" value="{_esc(contact.get("last_interaction_at"))}" /></div>
    ...
    *,
    ...
    main = f"""<section class="admin-section" aria-labelledby="contacts-title">
      ...
      <form class="admin-form" method="get" action="/admin/contacts">
        ...
        <div class="field"><label for="relationship-filter">Relationship</label><select id="relationship-filter" name="relationship_strength">{_options(RELATIONSHIP_STRENGTHS, filters.get("relationship_strength"))}</select></div>
