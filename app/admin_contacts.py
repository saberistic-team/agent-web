...
def _role_checkboxes(selected: list[str] | None) -> str:
    ...
    return "\n".join(
        f"""<label class="admin-checkbox">
          <input type="checkbox" name="buying_roles" value="{_esc(key)}"{" checked" if key in selected_set else ""} />
          {_esc(label)}
        </label>"""
...
def _contact_form(
    *,
    ...
    if last_interaction is not None and hasattr(last_interaction, "date"):
        last_interaction = last_interaction.date().isoformat()
    return f"""<form class="admin-form" method="post" action="{_esc(action)}">
      ...
      <div class="field"><label for="last_interaction_at">Last interaction</label><input id="last_interaction_at" name="last_interaction_at" type="date" value="{_esc(last_interaction)}" /></div>
...
def render_contacts_list_page(
    *,
    ...
    company_names = {str(row["id"]): str(row.get("name") or row["id"]) for row in companies}
    rows = "".join(
        f"""<tr>
          <td><a href="/admin/contacts/{_esc(row["id"])}/edit">{_esc(row.get("full_name"))}</a></td>
          <td>{_esc(row.get("title") or "—")}</td>
          <td>{_esc(company_names.get(str(row.get("company_id")), "—"))}</td>
    ...
    main = f"""<section class="admin-section" aria-labelledby="contacts-title">
      ...
      <form class="admin-form" method="get" action="/admin/contacts">
        <div class="field"><label for="q">Search</label><input id="q" name="q" value="{_esc(filters.get("q"))}" placeholder="Name, email, or title" /></div>
        ...
        <div class="field"><label for="strength-filter">Relationship</label><select id="strength-filter" name="relationship_strength">{_options(RELATIONSHIP_STRENGTHS, filters.get("relationship_strength"))}</select></div>
...
def render_contact_form_page(
    *,
    ...
    warning_html = "".join(
        f'<li>{_esc(item.reason)} already matches '
        f'<a href="/admin/contacts/{_esc(item.contact_id)}/edit">{_esc(item.full_name)}</a>.</li>'
