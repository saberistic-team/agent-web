...
class TrustedProxyConfig:
    ...
    @property
    def trust_wildcard(self) -> bool:
        """Backward-compatible alias for deployment summaries."""
        return self.trust_peer_wildcard
...
def build_trusted_proxy_config(cidrs: tuple[str, ...]) -> TrustedProxyConfig:
    ...
    if _WILDCARD_TRUST in cidrs:
        hop_networks = _DEFAULT_TRUSTED_HOP_NETWORKS + explicit_networks
        return TrustedProxyConfig(
            cidrs=cidrs,
    ...
    return TrustedProxyConfig(
        ...
        hop_networks=explicit_networks,
    )
