...
def build_preview_child_env(
    *,
    ...
    preview_seed = (parent.get(ENV_PREVIEW_SEED) or "").strip()
    if preview_seed:
        env[ENV_PREVIEW_SEED] = preview_seed
    preview_reference_time = (parent.get(ENV_PREVIEW_REFERENCE_TIME) or "").strip()
    if preview_reference_time:
        env[ENV_PREVIEW_REFERENCE_TIME] = preview_reference_time
    preview_fixture_version = (parent.get(ENV_PREVIEW_FIXTURE_VERSION) or "").strip()
    if preview_fixture_version:
        env[ENV_PREVIEW_FIXTURE_VERSION] = preview_fixture_version
    apply_preview_env_defaults(env)
...
def build_screenshot_reproducibility_record(
    *,
    phase: str,
    head_sha: str = "",
    browser_version: str = "",
    ...
    """Non-secret metadata so screenshot runs can be reproduced and compared."""
    ctx = preview_context or PreviewContext.from_environ()
    record: dict[str, Any] = {
        **ctx.reproducibility_fields(),
        "phase": phase,
        "head_sha": head_sha,
...
def format_reproducibility_markdown(record: dict[str, Any]) -> list[str]:
    ...
    viewports = record.get("viewports") or []
    if viewports:
        summary = ", ".join(
            f"{item['name']} ({item['width']}×{item['height']})" for item in viewports
        )
        lines.append(f"  - viewports: {summary}")
    extra = record.get("admin_extra_viewports") or []
    if extra:
        summary = ", ".join(
            f"{item['name']} ({item['width']}×{item['height']})" for item in extra
        )
        lines.append(f"  - admin_extra_viewports: {summary}")
    return lines
...
def capture(
    ...
    *,
    ...
    reproducibility = build_screenshot_reproducibility_record(
        phase=phase,
        head_sha=head_sha,
        browser_version=browser_version,
    )
    repro_report = out_dir / f"{phase}-reproducibility.json"
    repro_report.write_text(json.dumps(reproducibility, indent=2) + "\n", encoding="utf-8")
    return CaptureResult(
        paths=paths,
        overflows=overflows,
...
def comment_markdown_pre_dual(
    *,
    ...
    if reproducibility:
        lines.extend(format_reproducibility_markdown(reproducibility))
    lines.extend(
        [
            "- evidence (headless Chromium):",
    ...
    try:
        ...
        if dual_pre:
            ...
            if not routes:
                ...
                head_sha = (os.environ.get("GITHUB_SHA") or "").strip()
