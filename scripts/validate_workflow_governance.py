...
SUBPROCESS_SCRIPT_RE = re.compile(r"""['"]scripts/([A-Za-z0-9_]+\.py)['"]""")
...
def _imports_from_script(path: Path) -> set[str]:
    """Return sibling script module names imported or subprocess-invoked by ``path``."""
    text = path.read_text()
    try:
        tree = ast.parse(text)
    except SyntaxError:
    ...
    for match in IMPORTLIB_SCRIPT_RE.findall(text):
        for group in match:
            if group:
                modules.add(Path(group).stem)
    for match in SUBPROCESS_SCRIPT_RE.findall(text):
        modules.add(Path(match).stem)
    return modules
