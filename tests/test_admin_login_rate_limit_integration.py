...
def test_username_rotation_shares_source_bucket(pg_conn: psycopg.Connection) -> None:
    ...
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.10", _limiter_settings())
...
def test_concurrent_admission_does_not_overshoot_threshold(
    ...
    source_key = admin_auth.build_source_rate_limit_key("198.51.100.20", _limiter_settings())
...
def test_window_boundary_resets_failure_count(pg_conn: psycopg.Connection) -> None:
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.44", _limiter_settings())
    window_seconds = 60
...
def test_expired_lockout_allows_new_admissions(pg_conn: psycopg.Connection) -> None:
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.55", _limiter_settings())
...
def test_cleanup_removes_stale_unlocked_rows(pg_conn: psycopg.Connection) -> None:
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.66", _limiter_settings())
