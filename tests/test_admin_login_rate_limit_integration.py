...
def limiter_settings_env(monkeypatch: pytest.MonkeyPatch) -> Settings:
...
def test_username_rotation_shares_source_bucket(
    ...
    limiter_settings_env: Settings,
) -> None:
    now = datetime(2026, 1, 1, 12, 0, tzinfo=timezone.utc)
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.10", limiter_settings_env)

    for index in range(5):
        user_key = admin_auth.build_rate_limit_key(
            f"user-{index}", "203.0.113.10", limiter_settings_env
        )
...
def test_concurrent_admission_does_not_overshoot_threshold(
    ...
    source_key = admin_auth.build_source_rate_limit_key("198.51.100.20", limiter_settings_env)
...
def test_account_bucket_limits_configured_admin_across_sources(
    ...
    account_key = admin_auth.build_account_rate_limit_key("operator", limiter_settings_env)
    now = datetime(2026, 3, 1, 8, 0, tzinfo=timezone.utc)

    for index in range(5):
        source_key = admin_auth.build_source_rate_limit_key(
            f"203.0.113.{index + 1}", limiter_settings_env
        )
        admission = _admit(
            pg_conn,
    ...
    blocked_source = admin_auth.build_source_rate_limit_key(
        "203.0.113.99", limiter_settings_env
    )
    blocked = _admit(
        pg_conn,
...
def test_window_boundary_resets_failure_count(
    ...
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.44", limiter_settings_env)
    window_seconds = 60
...
def test_expired_lockout_allows_new_admissions(
    ...
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.55", limiter_settings_env)
...
def test_cleanup_removes_stale_unlocked_rows(
    ...
    source_key = admin_auth.build_source_rate_limit_key("203.0.113.66", limiter_settings_env)
