"""Integration tests for analytics dashboard queries."""
...
def _require_database_url() -> str:
    ...
    pytest.skip("TEST_DATABASE_URL not set; skipping analytics dashboard integration tests")
...
def test_dashboard_counts_known_fixture_events(pg_conn: psycopg.Connection) -> None:
    ...
    assert data.articles[0].slug == "postgres-indexing"
    assert data.articles[0].views == 1
    assert data.attribution[0].source == "linkedin"
    assert data.attribution[0].total_events == 2
