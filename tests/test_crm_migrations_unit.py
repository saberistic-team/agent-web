...
def test_apply_migrations_runs_only_pending_steps() -> None:
    ...
    assert any("audit_events" in sql for sql in execute_calls)
    assert any(
...
def test_audit_events_migration_is_append_only() -> None:
    ...
    assert audit.version == "005"
    assert "CREATE TABLE IF NOT EXISTS audit_events" in audit.up_sql
    assert "correlation_id TEXT NOT NULL" in audit.up_sql
    assert "prevent_audit_events_mutation" in audit.up_sql
    assert "audit_events_no_update" in audit.up_sql
    assert "audit_events_no_delete" in audit.up_sql
