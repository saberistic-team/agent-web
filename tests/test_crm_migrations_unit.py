...
def test_pending_migrations_skips_applied_versions() -> None:
    ...
    assert [m.version for m in pending] == [
        "003", "004", "005", "006", "007", "008", "009", "010", "011", "012", "013", "014", "015", "016", "017", "018", "019", "020", "021", "022", "023", "024", "025", "026",
    ]
...
def test_apply_migrations_runs_only_pending_steps() -> None:
    ...
    assert applied == ["003", "004", "005", "006", "007", "008", "009", "010", "011", "012", "013", "014", "015", "016", "017", "018", "019", "020", "021", "022", "023", "024", "025", "026"]
...
def test_apply_migrations_on_empty_database_applies_all() -> None:
    ...
    assert applied == ["001", "002", "003", "004", "005", "006", "007", "008", "009", "010", "011", "012", "013", "014", "015", "016", "017", "018", "019", "020", "021", "022", "023", "024", "025", "026"]
    conn.commit.assert_called_once()
...
@pytest.mark.unit
def test_import_batches_migration_is_idempotent() -> None:
    migration = next(m for m in MIGRATIONS if m.name == "import_batches")
    assert migration.version == "014"
    assert "CREATE TABLE IF NOT EXISTS import_batches" in migration.up_sql
    assert "CREATE TABLE IF NOT EXISTS import_batch_rows" in migration.up_sql
    assert "idx_import_batches_checksum_committed" in migration.up_sql


@pytest.mark.unit
def test_contact_field_sources_migration_is_idempotent() -> None:
    migration = next(m for m in MIGRATIONS if m.name == "contact_field_sources")
    assert migration.version == "019"
    assert "field_sources" in migration.up_sql
    assert "ADD COLUMN IF NOT EXISTS" in migration.up_sql


@pytest.mark.unit
def test_contact_relationship_metrics_migration_is_idempotent() -> None:
    migration = next(m for m in MIGRATIONS if m.name == "contact_relationship_metrics")
    assert migration.version == "022"
    assert "relationship_metrics" in migration.up_sql
    assert "crm_context_tags" in migration.up_sql


@pytest.mark.unit
def test_discovery_reconciliation_migration_is_idempotent() -> None:
    migration = next(m for m in MIGRATIONS if m.name == "discovery_reconciliation")
    assert migration.version == "025"
    assert "field_sources" in migration.up_sql
    assert "discovery_review_queue" in migration.up_sql
    assert "discovery_merge_decisions" in migration.up_sql


@pytest.mark.unit

def test_discovery_inbox_migration_tables() -> None:
    migration = next(m for m in MIGRATIONS if m.name == "discovery_inbox")
    assert migration.version == "026"
    assert "discovery_candidates" in migration.up_sql
    assert "discovery_rejection_suppressions" in migration.up_sql
    assert "discovery_runs" not in migration.up_sql.split("discovery_candidates")[0]
    # Reuses shipped discovery_runs from 024 rather than recreating it.
    assert "REFERENCES discovery_runs" in migration.up_sql
...
def test_concurrent_initializers_apply_each_migration_once(
    ...
    assert errors == []
    assert shared_db._applied_versions == {"001", "002", "003", "004", "005", "006", "007", "008", "009", "010", "011", "012", "013", "014", "015", "016", "017", "018", "019", "020", "021", "022", "023", "024", "025", "026"}
...
@pytest.mark.unit
def test_first_party_analytics_events_migration_is_idempotent() -> None:
    migration = next(m for m in MIGRATIONS if m.name == "first_party_analytics_events")
    assert migration.version == "017"
    assert "CREATE TABLE IF NOT EXISTS analytics_events" in migration.up_sql
    assert "analytics_events_idempotency_key_unique" in migration.up_sql
    assert "CREATE TABLE IF NOT EXISTS analytics_sessions" in migration.up_sql
    assert "CREATE TABLE IF NOT EXISTS analytics_event_rate_limits" in migration.up_sql
