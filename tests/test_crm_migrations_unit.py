...
def test_pending_migrations_skips_applied_versions() -> None:
    ...
    assert [m.version for m in pending] == ["003", "004", "005", "006", "007", "008", "009", "010", "011"]
...
def test_apply_migrations_runs_only_pending_steps() -> None:
    ...
    assert applied == ["003", "004", "005", "006", "007", "008", "009", "010", "011"]
...
def test_apply_migrations_on_empty_database_applies_all() -> None:
    ...
    assert applied == ["001", "002", "003", "004", "005", "006", "007", "008", "009", "010", "011"]
    conn.commit.assert_called_once()
...
@pytest.mark.unit
def test_company_records_migration_is_idempotent() -> None:
    company_records = next(m for m in MIGRATIONS if m.name == "company_records")
    assert company_records.version == "010"
    assert "domain TEXT" in company_records.up_sql
    assert "archived_at TIMESTAMPTZ" in company_records.up_sql
    assert "idx_companies_domain" in company_records.up_sql
...
def test_concurrent_initializers_apply_each_migration_once(
    ...
    assert errors == []
    assert shared_db._applied_versions == {"001", "002", "003", "004", "005", "006", "007", "008", "009", "010", "011"}
