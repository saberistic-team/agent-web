...
def test_parse_connections_csv_with_notes_preamble() -> None:
    """Current LinkedIn exports prepend a Notes: disclaimer before the header."""
    data = _build_export_zip({"Connections.csv": CONNECTIONS_CSV_WITH_NOTES_PREAMBLE})
    result = parse_linkedin_export_zip(data)
    assert result.ok is True
...
@pytest.mark.unit
@pytest.mark.integration
def test_rejects_headerless_csv_when_preamble_scan_exceeded() -> None:
