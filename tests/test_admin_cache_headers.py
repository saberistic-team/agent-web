...
@pytest.mark.integration
def test_admin_pipeline_preview_guard_405_has_cache_headers(preview_mode: None) -> None:
    response = client.post(
...
@pytest.mark.integration
def test_admin_500_handler_has_cache_headers() -> None:
    def _boom(*args: Any, **kwargs: Any) -> Response:
        raise RuntimeError("simulated admin failure")

    error_client = TestClient(app, follow_redirects=False, raise_server_exceptions=False)
    with mock_db_connection():
        with patch("app.admin_routes._issue_login_flow_response", side_effect=_boom):
            response = error_client.get("/admin/login")
...
def test_login_failed_post_has_cache_headers() -> None:
    from tests.test_admin_auth import _extract_csrf_token, _fetch_login_form
...
def test_logout_redirect_has_cache_headers() -> None:
    ...
    _session_store[token_hash] = {
        **_session_row(token_hash=token_hash),
    ...
    with (
        patch.object(db, "get_admin_session_by_token_hash", side_effect=lambda _c, th: _session_store.get(th)),
        patch("app.admin_routes.db.get_admin_session_by_token_hash", side_effect=lambda _c, th: _session_store.get(th)),
        patch("app.admin_routes.db.revoke_admin_session", side_effect=_mock_revoke_admin_session),
        mock_db_connection(),
    ):
        response = client.post(
            "/admin/logout",
...
@pytest.mark.integration
def test_static_assets_do_not_inherit_admin_cache_policy() -> None:
...
@pytest.mark.integration
def test_admin_cache_header_appears_once(preview_mode: None) -> None:
    response = client.get("/admin/login")
...
@pytest.mark.integration
def test_route_cannot_weaken_admin_cache_policy() -> None:
    from app import admin_routes

    original = admin_routes._issue_login_flow_response

    def _weak_cache_response(*args: Any, **kwargs: Any) -> Response:
    ...
    with mock_db_connection():
        with patch.object(
            admin_routes,
            "_issue_login_flow_response",
            side_effect=_weak_cache_response,
        ):
            response = client.get("/admin/login")
    assert response.status_code == 200
    _assert_admin_cache_headers(response)
