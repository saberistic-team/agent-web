...
@pytest.mark.unit
def test_admit_admin_login_attempt_executes_transactional_upsert() -> None:
    conn = MagicMock()
    cur = MagicMock()
    ...
    assert admitted
    assert not newly_locked
    assert any("FOR UPDATE" in call.args[0] for call in cur.execute.call_args_list)
