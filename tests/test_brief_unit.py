...
def test_extract_payment_from_session_full_price() -> None:
    ...
    assert payment.amount_subtotal_cents == 20_000
    assert payment.amount_discount_cents == 0
    assert payment.amount_total_cents == 20_000
    ...
    payment = stripe_service.extract_payment_from_session(
        {
            ...
            "currency": "usd",
            "total_details": {
                "amount_discount": 10_000,
                "breakdown": {
                    "discounts": [
                        {
                            "discount": {
                                "promotion_code": "promo_test_half",
                                "coupon": "coupon_test_half",
                            }
                        }
                    ]
                },
            },
            "discounts": [{"promotion_code": "promo_test_half", "coupon": "coupon_test_half"}],
        }
    )
    assert payment.amount_discount_cents == 10_000
    assert payment.amount_total_cents == 10_000
    assert payment.stripe_promotion_code_id == "promo_test_half"
    assert payment.stripe_coupon_id == "coupon_test_half"


@pytest.mark.unit
def test_extract_payment_from_session_computes_discount_when_missing() -> None:
    payment = stripe_service.extract_payment_from_session(
        {
            "amount_subtotal": 20_000,
            "amount_total": 0,
    ...
    assert payment.amount_discount_cents == 20_000
    assert payment.amount_total_cents == 0
