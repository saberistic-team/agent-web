...
@pytest.mark.unit
def test_client_ip_uses_rightmost_untrusted_xff_when_peer_trusted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.unit
@pytest.mark.integration
def test_rate_limit_uses_parsed_client_when_peer_trusted(
    rate_limit_store: FakeRateLimitStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ADMIN_TRUSTED_PROXY_CIDRS", "10.0.0.0/8")
    ...
    with shared_rate_limiter(rate_limit_store):
        with mock_db_connection():
            ...
            fresh_form = proxy_client.get("/admin/login")
            fresh_csrf = _extract_csrf_token(fresh_form.text)
            fresh_cookies = {
                LOGIN_FLOW_COOKIE_NAME: fresh_form.cookies[LOGIN_FLOW_COOKIE_NAME]
            }
