...
@pytest.mark.unit
@pytest.mark.integration
def test_rotating_spoofed_leftmost_headers_share_one_limiter_record(
    rate_limit_store: FakeRateLimitStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ADMIN_TRUST_PROXY_HEADERS", "true")
    ...
    with shared_rate_limiter(rate_limit_store):
        for index in range(4):
            ...
            request.headers.__dict__["_list"].append(
                (
                    b"x-forwarded-for",
                    f"203.0.113.{index}, 203.0.113.50".encode("ascii"),
                )
            )
...
@pytest.mark.unit
@pytest.mark.integration
def test_rate_limit_ignores_spoofed_forwarded_when_trust_enabled_but_peer_untrusted(
    rate_limit_store: FakeRateLimitStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """TestClient peer is not a trusted proxy; X-Forwarded-For must not rotate buckets."""
    monkeypatch.setenv("ADMIN_TRUST_PROXY_HEADERS", "true")
    monkeypatch.setenv("ADMIN_TRUSTED_PROXY_IPS", "10.0.0.0/8")
    monkeypatch.setenv("ADMIN_LOGIN_RATE_LIMIT", "2")
    with shared_rate_limiter(rate_limit_store):
        headers_a = {"X-Forwarded-For": "203.0.113.77"}
        assert _login(username="ghost", password="wrong", headers=headers_a).status_code == 401
        assert _login(username="ghost", password="wrong", headers=headers_a).status_code == 401
        assert _login(username="ghost", password="wrong", headers=headers_a).status_code == 429

        other_ip_headers = {"X-Forwarded-For": "203.0.113.88"}
        assert _login(username="ghost", password="wrong", headers=other_ip_headers).status_code == 429
