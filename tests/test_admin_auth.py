...
@pytest.mark.unit
def test_client_ip_uses_trusted_hop_walk_when_proxy_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    request.headers.__dict__["_list"].append(
        (b"x-forwarded-for", b"198.51.100.99, 203.0.113.50")
    )
    ...
    def _trusted_login(**kwargs: Any) -> Any:
        ...
        if "X-Forwarded-For" in headers:
            headers["X-Forwarded-For"] = (
                f"198.51.100.99, {headers['X-Forwarded-For']}"
            )

        def _patched_resolve(request: Request, settings: Any) -> Any:
            if request.client is not None and request.client.host == "testclient":
                peered = _request_with_client("10.0.0.1")
                for name, value in request.headers.raw:
                    peered.headers.__dict__["_list"].append((name, value))
                return resolve_admin_login_client_source(peered, settings)
        ...
        with patch(
            ...
            return _login(headers=headers, **kwargs)

    with shared_rate_limiter(rate_limit_store):
        assert _trusted_login(username="ghost", password="wrong", headers={"X-Forwarded-For": "203.0.113.77"}).status_code == 401
        assert _trusted_login(username="ghost", password="wrong", headers={"X-Forwarded-For": "203.0.113.77"}).status_code == 401
        assert _trusted_login(username="ghost", password="wrong", headers={"X-Forwarded-For": "203.0.113.77"}).status_code == 429
