...
def admin_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("ADMIN_TRUSTED_FORWARDING_CIDRS", raising=False)
    monkeypatch.delenv("ADMIN_TRUST_CLOUDFLARE_FORWARDING", raising=False)
...
def test_client_ip_uses_forwarded_when_trusted_proxy_enabled(
    ...
    monkeypatch.setenv("ADMIN_TRUSTED_FORWARDING_CIDRS", "10.0.0.0/8")
    settings = get_settings()
...
def test_rate_limit_uses_forwarded_ip_when_trusted(
    ...
    monkeypatch.setenv("ADMIN_TRUSTED_FORWARDING_CIDRS", "testclient")
    monkeypatch.setenv("ADMIN_LOGIN_RATE_LIMIT", "2")
    with shared_rate_limiter(rate_limit_store):
...
@pytest.mark.unit
@pytest.mark.integration
def test_rate_limit_rotating_spoofed_headers_single_source_bucket(
    rate_limit_store: FakeRateLimitStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ADMIN_LOGIN_RATE_LIMIT", "3")
    ...
    assert len(rate_limit_store.rows) == 1
    assert admin_auth.build_source_rate_limit_key("testclient") in rate_limit_store.rows


@pytest.mark.unit
@pytest.mark.integration
def test_rate_limit_persisted_rows_contain_no_raw_forwarding_data(
    rate_limit_store: FakeRateLimitStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("ADMIN_TRUSTED_PROXY_CIDRS", "testclient")
    ...
    serialized = json.dumps(rate_limit_store.rows, default=str)
    assert "203.0.113.55" not in serialized
    assert "203.0.113.66" not in serialized
