...
def test_build_rate_limit_key_hashes_username_and_source() -> None:
    settings = get_settings()
    key_a = admin_auth.build_rate_limit_key("Operator", "203.0.113.1", settings=settings)
    key_b = admin_auth.build_rate_limit_key("operator", "203.0.113.1", settings=settings)
    key_c = admin_auth.build_rate_limit_key("operator", "203.0.113.2", settings=settings)
    assert key_a == key_b
    assert key_a != key_c
    assert len(key_a) == 64
