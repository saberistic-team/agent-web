...
def test_contact_record_mutations_require_session_and_use_csrf() -> None:
    unauthenticated = client.post(
        "/admin/contacts",
        data={"csrf_token": CSRF_TOKEN, "full_name": "Ada"},
    )
    assert unauthenticated.status_code == 303

    created = {**_contact, "full_name": "Ada Lovelace", "buying_roles": ["founder"]}
    with patch("app.admin_routes.require_admin_session", return_value=_fake_session()):
        with patch("app.admin_routes._crm") as crm:
...
def test_contact_new_edit_restore_and_invalid_fields_are_handled() -> None:
    ...
    with patch("app.admin_routes.require_admin_session", return_value=_fake_session()):
        with patch("app.admin_routes._crm") as crm:
            ...
            assert edit_page.status_code == 200 and "Edit Ada Lovelace" in edit_page.text
            invalid = client.post(
                f"/admin/contacts/{CONTACT_ID}/edit",
                data={"csrf_token": CSRF_TOKEN, "full_name": "Ada", "buying_roles": ["not-real"]},
            )
            assert invalid.status_code == 303 and "error=" in invalid.headers["location"]
            updated = client.post(
                f"/admin/contacts/{CONTACT_ID}/edit",
            ...
            assert updated.status_code == 303
            assert crm.update_contact.call_args.kwargs["contact"].buying_roles == ["influencer"]
            restored = client.post(
