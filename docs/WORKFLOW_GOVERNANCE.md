...
2. walks transitive imports and subprocess invocations from those entrypoints
   and from manifest-covered scripts; and
...
`python scripts/validate_workflow_governance.py --check-live-ruleset` and fails
if any existing protected file has no owner, has a bot owner, or sits outside
