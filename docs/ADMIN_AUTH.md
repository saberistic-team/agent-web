...
| `ADMIN_PREVIEW_SEED` | Optional | Root seed for namespace-derived preview RNG. Screenshot CI uses checked-in default `338`; tests may override explicitly. Malformed values fail fast. |
| `ADMIN_PREVIEW_REFERENCE_TIME` | Optional | Frozen timezone-aware reference timestamp (ISO 8601) for preview time-derived fields. Screenshot CI uses `2026-07-14T12:00:00+00:00` by default. |
| `ADMIN_PREVIEW_FIXTURE_VERSION` | Optional | Preview fixture schema version (default `1`). Bump when regenerating screenshot baselines after fixture changes. |
