...
| `ADMIN_PREVIEW_SEED` | Optional | Root seed for mock admin randomization. Screenshot launcher sets stable default `338001`; override for local visual testing. |
| `ADMIN_PREVIEW_REFERENCE_TIME` | Optional | Frozen ISO-8601 UTC timestamp for time-derived preview fields. Screenshot launcher sets a stable default; override for local testing. |
| `ADMIN_PREVIEW_FIXTURE_VERSION` | Optional | Preview fixture schema version (default `1`). Bump when preview data shape changes intentionally. |
