...
| `ADMIN_LOGIN_LIMITER_SECRET` | Yes | HMAC-SHA-256 key for login limiter identifiers (≥ 32 random bytes; environment-specific) |
| `ADMIN_LOGIN_LIMITER_SECRET_PREVIOUS` | Optional | Previous limiter key during a bounded rotation window (see below) |
...
Generate a **separate** login limiter secret (never reuse the session secret):
...
effective rate-limit history may reset unless a rotation window is used.
...
4. Redeploy. Throttle checks honor both keys during the window; new admissions
   increment only the current key.
5. After lockout windows and retention have elapsed, clear
   ``ADMIN_LOGIN_LIMITER_SECRET_PREVIOUS`` and redeploy again.

Expired rows under either key remain eligible for bounded cleanup. Use separate
secrets in test, preview, and production environments.

**Controlled reset (acceptable alternative):**

1. Update ``ADMIN_LOGIN_LIMITER_SECRET`` without setting ``_PREVIOUS``.
2. Redeploy and accept that active rate-limit counters reset immediately.
3. Optionally truncate ``admin_login_rate_limits`` during a maintenance window
   if stale SHA-256 rows from pre-HMAC deployments should be removed.
...
Digests use ``ADMIN_LOGIN_LIMITER_SECRET`` (HMAC-SHA-256). A table reader without
the secret cannot verify guessed IP addresses or usernames by hashing them
directly. Domain prefixes (`src`, `acct`) prevent cross-family collisions.
